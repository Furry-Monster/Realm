...
def check_cmake():
    ...
    try:
        result = config.run_command(["cmake", "--version"], capture_output=True, check=False)
...
def check_compiler():
    ...
    if config.platform.value == "Windows":
        ...
        if config.find_program("g++"):
            try:
                result = config.run_command(["g++", "--version"], capture_output=True, check=False)
                if result.returncode == 0:
                    version = result.stdout.splitlines()[0]
        ...
        if config.find_program("clang++"):
            try:
                result = config.run_command(
                    ["clang++", "--version"], capture_output=True, check=False
                )
                if result.returncode == 0:
                    version = result.stdout.splitlines()[0]
                    logger.success(f"Found {version}")
