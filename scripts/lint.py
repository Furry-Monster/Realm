...
def find_source_files(src_dir: Path, extensions: Optional[list] = None) -> list:
...
def lint_files(
    ...
    for i, file in enumerate(files, 1):
        ...
        try:
            result = config.run_command([*cmd, str(file)], capture_output=True, check=False)
...
Examples:
  ...
  python lint.py -c "modernize-*"     # Use specific checks
    ...
    parser.add_argument("--clang-tidy", type=str, help="Path to clang-tidy executable")
    ...
    # Find clang-tidy
    clang_tidy = args.clang_tidy or config.find_program("clang-tidy")
    ...
    # Get version
    try:
        result = config.run_command([clang_tidy, "--version"], capture_output=True, check=False)
        if result.returncode == 0:
