...
def clean_pattern(directory: Path, pattern: str, dry_run: bool = False, verbose: bool = False):
    ...
    if not files:
        if verbose:
            logger.info(f"No files matching '{pattern}' in {directory.relative_to(config.project_root)}")
        return 0
...
Examples:
  ...
  python clean.py -v                # Verbose output
    ...
    parser.add_argument(
        "--bin",
        action="store_true",
        help="Clean binary output directory"
    )
    
    parser.add_argument(
        "--cache",
        action="store_true",
        help="Clean cache files (.cache, CMake cache)"
    )
