    ...
    for i, file in enumerate(files, 1):
        ...
        try:
            if check_only:
                ...
                result = config.run_command(
                    [clang_format, "-i", str(file)], capture_output=True, check=False
                )
                if result.returncode == 0:
                    formatted += 1
                else:
...
Examples:
  ...
  python format.py -v               # Verbose output
    ...
    parser.add_argument("--clang-format", type=str, help="Path to clang-format executable")
    ...
    # Find clang-format
    clang_format = args.clang_format or config.find_program("clang-format")
    ...
    # Get version
    try:
        result = config.run_command([clang_format, "--version"], capture_output=True, check=False)
        if result.returncode == 0:
