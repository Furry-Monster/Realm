...
void Renderer::initialize() {
    ...
    const RendererConfig &render_config =
        g_context.m_config->getRendererConfig();
    ...
    m_camera->lookAt(glm::vec3(render_config.camera_look_at_x,
                               render_config.camera_look_at_y,
                               render_config.camera_look_at_z));
    ...
    m_bloom_direction =
        static_cast<BloomDirection>(render_config.bloom_direction);
    ...
    m_pbr_framebuffer = std::make_unique<PBRFramebuffer>(m_window->getWidth(),
                                                         m_window->getHeight());
    ...
    m_bloom_framebuffers[0] = std::make_unique<BloomFramebuffer>(
        m_window->getWidth(), m_window->getHeight());
    m_bloom_framebuffers[0]->init();
    m_bloom_framebuffers[1] = std::make_unique<BloomFramebuffer>(
        m_window->getWidth(), m_window->getHeight());
...
void Renderer::compileShaders() {
    ...
    m_pbr_shader = std::make_unique<Shader>(vertex_path.generic_string(),
                                            fragment_path.generic_string());

    vertex_path = m_shader_path / "bloom.vert";
    fragment_path = m_shader_path / "bloom.frag";
    m_bloom_shader = std::make_unique<Shader>(vertex_path.generic_string(),
                                              fragment_path.generic_string());

    vertex_path = m_shader_path / "post.vert";
    fragment_path = m_shader_path / "post.frag";
    m_post_shader = std::make_unique<Shader>(vertex_path.generic_string(),
                                             fragment_path.generic_string());

    vertex_path = m_shader_path / "skybox.vert";
    fragment_path = m_shader_path / "skybox.frag";
    m_skybox_shader = std::make_unique<Shader>(vertex_path.generic_string(),
                                               fragment_path.generic_string());
}
...
void Renderer::precomputeIBL() {
    ...
    std::string hdri_path =
        (m_asset_path / render_config.hdri_path).generic_string();

    // Pre-compute IBL stuff
    m_ibl_equirectangular_cubemap =
        std::make_unique<EquirectangularCubemap>(root_path, hdri_path);
    m_ibl_equirectangular_cubemap->compute();

    // diffuse term
    m_ibl_diffuse_irradiance_map = std::make_unique<DiffuseIrradianceMap>(
        root_path, m_ibl_equirectangular_cubemap->getCubemapId());
    m_ibl_diffuse_irradiance_map->compute();

    // specular term
    m_ibl_specular_map = std::make_unique<SpecularMap>(
        root_path, m_ibl_equirectangular_cubemap->getCubemapId());
    ...
    m_ibl_skybox =
        std::make_unique<Skybox>(m_ibl_equirectangular_cubemap->getCubemapId());
}
...
void Renderer::render() {
    ...
    const RendererConfig &render_config =
        g_context.m_config->getRendererConfig();
    glClearColor(render_config.clear_color_r, render_config.clear_color_g,
                 render_config.clear_color_b, render_config.clear_color_a);
    ...
    for (size_t i = 0; i < std::min(m_render_scene->m_light_positions.size(),
                                    static_cast<size_t>(4));
         ++i)
        light_positions[i] = m_render_scene->m_light_positions[i];
    for (size_t i = 0; i < std::min(m_render_scene->m_light_colors.size(),
                                    static_cast<size_t>(4));
         ++i)
    ...
    m_pbr_shader->setInt("diffuseIrradianceMap",
                         TEXTURE_UNIT_DIFFUSE_IRRADIANCE_MAP);
    glBindTexture(GL_TEXTURE_CUBE_MAP,
                  m_ibl_diffuse_irradiance_map->getCubemapId());
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP,
                  m_ibl_specular_map->getPrefilteredEnvMapId());

    glActiveTexture(GL_TEXTURE0 + TEXTURE_UNIT_BRDF_CONVOLUTION_MAP);
    m_pbr_shader->setInt("brdfConvolutionMap",
...
void Renderer::renderSkybox() {
    ...
    m_skybox_shader->setModelViewProjectionMatrices(skybox_model, skybox_view,
                                                    skybox_proj);
    m_skybox_shader->setInt("skybox", 0);
    m_skybox_shader->setFloat("bloomBrightnessCutoff",
                              m_bloom_brightness_cutoff);
...
void Renderer::renderBloom() {
    ...
    for (auto mip_level = 0; mip_level <= 5; mip_level++) {
        ...
        glBindTexture(GL_TEXTURE_2D,
                      m_pbr_framebuffer->getBloomColorTextureId());
        ...
        for (auto i = 1; i < m_bloom_iterations; i++) {
            ...
            auto blur_direction =
                bloom_framebuffer == 1 ? blur_direction_y : blur_direction_x;
            m_bloom_shader->setVec2("blurDirection", blur_direction);
            glBindTexture(
                GL_TEXTURE_2D,
                m_bloom_framebuffers[source_buffer]->getColorTextureId());
...
void Renderer::applyPostprocess() {
    ...
    glBindTexture(GL_TEXTURE_2D,
                  m_bloom_framebuffers[m_bloom_result_id]->getColorTextureId());
