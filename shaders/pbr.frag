...
float subsurfaceWrap(float nDotL, float wrap) {
    return (nDotL + wrap) / (1.0 + wrap);
}
    ...
    vec3 subsurfaceColor)
{
    ...
    float nDotL = dot(n, l);
    // subsurfaceWrap needs raw nDotL (including negative) to produce the wrap effect
    float diffWeight = subsurfaceEnabled ? max(subsurfaceWrap(nDotL, subsurfaceRadius * 0.5), 0.0) : max(nDotL, 0.0);
